...
candidate = logical(validationOK) & T.ClassificationEligible & ...
    T.HasDiffusionEvidence;
T.ClassificationCode(candidate) = 1;
T.ClassificationLabel(candidate) = ...
    repmat({'BrownianCandidate'}, sum(candidate), 1);
T.ClassificationConfidence(candidate) = ...
    T.DiffusionFitRSquared(candidate);
T.ClassificationSuccessful(candidate) = true;
T.ClassificationBasis(candidate) = ...
    repmat({'BaselineBrownianFit'}, sum(candidate), 1);
